...
class ClickHouseDialect(default.DefaultDialect):
    ...
    def has_table(self, connection, table_name, schema=None, **kw):
        ...
        return any(r.result == 1 for r in self._execute(connection, query))
    ...
    def _get_column_type(self, name, spec):
        if spec.startswith("Array"):
            ...
            inner_types = [self._get_column_type(name, param) for param in inner]
            ...
            inner_types = [
                self._get_column_type(name, t.strip()) for t in inner.split(",")
            ]
            ...
            inner_types = [
                self._get_column_type(name, t.strip()) for t in inner.split(",", 1)
            ]
            return coltype(*inner_types)
    ...
    def get_pk_constraint(self, connection, table_name, schema=None, **kw):
        ...
        if schema:
            ...
            query = ("SELECT primary_key FROM system.tables WHERE name='{}'").format(
                table_name
            )
    ...
    def get_engine(self, connection, table_name, schema=None, **kw):
        ...
        query = text(
            ...
            "WHERE database = :database AND name = :name".format(", ".join(columns))
        )

        rows = self._execute(connection, query, database=database, name=table_name)
    ...
    def _get_server_version_info(self, connection):
        ...
        if version is None:
            version = self._execute(connection, "select version()", scalar=True)
    ...
    def _get_default_schema_name(self, connection):
        return self._execute(connection, "select currentDatabase()", scalar=True)
